...
function course_near_check(){
   ...
   course_data.forEach(course => {
       ...
       if(course['daily_avali']==0||course['course_week_type']==week_type){
          if(dic_weekday[course['course_weekday']]==current_weekday){
             if(current_minute>=dic_course_startmin['course_start']-10&&current_minute<=dic_course_startmin['course_start']+10){
                course_block(course,current_minute-dic_course_startmin['course_start']);
             }
          }
       }
   }
...
<span class='task-config'>
   <span class="task-config-content">
      <div class='task-config-content-left'>
         <p>完成</p>
      </div>
      <div class='task-config-content-right'>
         <p>标记为重要</p>
