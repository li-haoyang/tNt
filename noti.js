function noti_permit(){
  ...
  noti_permit_fail_jbox.open();
        ...
        var noti_permit_success= new Notification("通知已打开！",{icon:"https://migu.plus/t/pic/crm.png",tag: 'noti_permit_done'});
    };
}
function noti_course(){
    ...
    for(var i=1;i<=13;i++){
        if(current_m==dic_course_startmin[i]){
            if (window.Notification && Notification.permission== "granted"){
            var noti_course_start= new Notification("开始上第"+i+"节课啦",{icon:"https://migu.plus/t/pic/icon_course_start.png",tag: 'noti_course_start'});
setTimeout(noti_course_start.close.bind(noti_course_start), 4000);
...
jbox_notice_1("第"+i+"节课开始上课咯~",2,"left","fancy");
            ...
            if (window.Notification && Notification.permission== "granted"){
                var noti_course_finish= new Notification("第"+i+"节课已结束",{icon:"https://migu.plus/t/pic/noti_course_finish.png",tag: 'noti_course_finish'});
setTimeout(noti_course_finish.close.bind(noti_course_finish), 4000);                  
}else{
...
function course_near_noti(){
    current_minute=get_minute(-1);
    course_obj.forEach(course => {
        if(course['daily_avali']==1){
              if(current_minute==dic_course_startmin[course['course_start']]-5){
                var course_near_noti= new Notification(course['course_name']+"还有五分钟就要上课啦",{body:"查看课程表",icon:"https://migu.plus/t/pic/course_near_noti.png",tag: 'course_near_noti'});
              }else if(current_minute==dic_course_startmin[course['course_start']]){
                var course_begin_noti= new Notification(course['course_name']+"已经开始啦",{body:"前往课室签到",icon:"https://migu.plus/t/pic/course_near_noti.png",tag: 'course_near_noti'});
